...
class Page(tk.Frame):
    def __init__(self, master, name):
        ...
        if name == "Page 1":
            ...
            self.canvas = tk.Canvas(container, width=300, height=300)
            self.canvas.grid_forget()

    def show_option(self,):
        self.labelBottom.grid(row=4, column=0, sticky='w', padx=10, pady=5)
        self.labelUpper.grid(row=5, column=0, sticky='w', padx=10, pady=5)
        self.entryBottomLimit.grid(row=4, column=1, columnspan=1, padx=10, pady=5)
        self.entryUpperLimit.grid(row=5, column=1, columnspan=1, padx=10, pady=5)
        self.buttonUp.grid(row=6, column=0, columnspan=1, padx=10, pady=10)
        self.button.grid_forget()
        img = ImageTk.PhotoImage(Image.open("chart.png"))
        self.canvas.create_image(20, 20, image=img)
        self.canvas.grid(row=7, column=1, columnspan=1, padx=10, pady=10)
    ...
    def plotChartButtonCLicked(self):
        ...
        try:
            ...
            answer = messagebox.askquestion(title="Brak podanej częstotliwośći",
                                            message="Nie podano częstotliwości. Czy chcesz przyjąć, że częstotliwość to 100 Hz?")
