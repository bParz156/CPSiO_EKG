...
class Page(tk.Frame):
    def __init__(self, master, name):
        ...
        if name == "Page 1":
            ...
            fourierImgae = tk.PhotoImage(file="tfouriera.png")
            self.fourierImgae = tk.Label(self.frame2, image=fourierImgae)
            self.fourierImgae.image = fourierImgae  
            self.fourierImgae.pack()
            ...
            self.clicked=True
            if(not xarray):
                self.labelEmpty=tk.Label(container, text="Najpierw musisz wejsc w zadanie 1 i wczytac sygnal", font=("Helvetica", 18))
                self.labelEmpty.grid(row=0, column=0, sticky='w', padx=10, pady=5)
                self.clicked=False
                self.buttonRead=tk.Button(container,text="Kliknij, jesli chcesz wczytac plik ekg100", command=self.zadanie3)
                self.buttonRead.grid(row=1, column=1, sticky='w', padx=10, pady=5)
    ...
    def zadanie3(self):
        if not self.clicked:
            zadanie1(freq=360,filename="ekg100.txt", hasFirstTime=False)
        transformaty()
        self.buttonRead.grid_forget()
        self.labelEmpty.grid_forget()
        imageTF = tk.PhotoImage(file="tf.png")
        canvas=tk.Canvas(self)
        canvas.pack(side="left", fill="both", expand=True)
        frame = tk.Frame(canvas)
        canvas.create_window((0,0), window=frame, anchor="nw")
        imageTFLabel = tk.Label(frame, image=imageTF, text="Transformata Fouriera")
        imageTFLabel.image = imageTF  
        imageTFLabel.grid()


        imageITF = tk.PhotoImage(file="itf.png")
        canvas2=tk.Canvas(self)
        canvas2.pack(side="left", fill="both", expand=True)
        frame2 = tk.Frame(canvas2)
        canvas2.create_window((0,1), window=frame2, anchor="nw")
        imageITFLabel = tk.Label(frame, image=imageITF, text="Transformata odwrotna Fouriera")
        imageITFLabel.image = imageITF  
        imageITFLabel.grid()

        imageDIF = tk.PhotoImage(file="roznica.png")
        canvas3=tk.Canvas(self)
        canvas3.pack(side="left", fill="both", expand=True)
        frame3 = tk.Frame(canvas3)
        canvas3.create_window((0,2), window=frame3, anchor="nw")
        imageDIFLabel = tk.Label(frame, image=imageDIF, text="Transformata odwrotna Fouriera")
        imageDIFLabel.image = imageITF  
        imageDIFLabel.grid()
...
def zad2(n, frequency):
    fs=frequency*2
    Ts=1/fs
    x=np.linspace(start=0, stop=n,num=n, endpoint=False)
    x=x*Ts
    k=2*np.pi*frequency*x
    y=np.sin(k)
    plt.subplots_adjust(hspace=1.0, wspace=4.0, right=1.0)  # Większe odstępy między wykresami
    plt.subplot(6, 1, 1)
    plt.plot(k[0:int(len(y)/1024)],y[0:int(len(y)/1024)])
    plt.xlabel('Czas [s]')
    plt.grid(True)
    plt.subplot(6, 1, 2)
    fx=x[0:int(fs/2)]
    tf=np.fft.fft(y) #transformata fouriera
    valuetx=tf[0:int(fs/2)]
    valuetx=abs(valuetx)
    plt.plot(valuetx, fx)
    ...
    plt.tight_layout()  # Automatyczna regulacja odstępów
    plt.savefig(fname="tfouriera.png")
...
def transformaty():
    ...
    rightEdge=int(fs/2)
    n=len(valuesArray[0])
    tf=np.fft.fft(valuesArray[0],n)
    modul=abs(tf)
    odwrotna=np.fft.ifft(tf)
    odwrotnamod=abs(odwrotna)
    roznica=valuesArray[0]-odwrotna
    
    plt.figure()  # Ustawienie większego rozmiaru figury
    plt.plot(xarray[0:rightEdge], modul[0:rightEdge], label='Transformata Foueriera')
    plt.xlabel('Czestotliwosc [Hz]')
    plt.legend(loc='upper right')
    plt.tight_layout()  # Automatyczna regulacja odstępów
    plt.grid(True)
    plt.savefig(fname="tf.png")

    plt.figure()  # Ustawienie większego rozmiaru figury
    plt.plot(xarray[0:rightEdge], odwrotnamod[0:rightEdge], label='Odwrotna')
    plt.xlabel('Czas [s]')
    plt.legend(loc='upper right')
    plt.tight_layout()  # Automatyczna regulacja odstępów
    plt.grid(True)
    plt.savefig(fname="itf.png")

    plt.figure()  # Ustawienie większego rozmiaru figury
    plt.plot(xarray[0:rightEdge], roznica[0:rightEdge], label='Roznica z oryginalem')
    plt.xlabel('czas [s]')
    plt.legend(loc='upper right')
    plt.tight_layout()  # Automatyczna regulacja odstępów
    plt.grid(True)
    plt.savefig(fname="roznica.png")
